...
export const Toolbar: React.FC<ToolbarProps> = ({
  ...
  return (
    <div className="w-80 bg-gray-800 border-r border-gray-700 flex flex-col h-full shadow-xl z-10 shrink-0">
      <div className="p-4 border-b border-gray-700">
        ...
        <div className="flex gap-2 mt-4 bg-gray-900 p-1 rounded-lg">
          <button 
            ...
            className={`flex-1 py-1 px-2 text-xs font-bold rounded transition-all ${activeTab === 'edit' ? 'bg-gray-700 text-white' : 'text-gray-500 hover:text-gray-300'}`}
          >
            Mesa
          </button>
          <button 
             onClick={() => setActiveTab('multiplayer')}
            className={`flex-1 py-1 px-2 text-xs font-bold rounded transition-all flex justify-center items-center gap-1 ${activeTab === 'multiplayer' ? 'bg-indigo-900 text-indigo-100' : 'text-gray-500 hover:text-gray-300'}`}
          >
